...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVPTest-V3.15.511gke-%PROC%%X%-%T%%G%%R%%E%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVPTest-V3.15.511gke-%PROC%%X%-%D%%T%%G%%R%%E%.hex
echo compiled - UAVPTest-V3.15.511gke-%PROC%%X%-%D%%T%%G%%R%%E%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVPTest-V3.15.511gke-%PROC%%X%-%T%%G%%R%%E%.hex
echo failed - UAVPTest-V3.15.511gke-%PROC%%X%-%T%%G%%R%%E%.hex >> gen.lst
