...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVP-V3.15.$WCREV$gke-%PROC%%X%-%C%%D%%T%%G%%R%%E%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVP-V3.15.$WCREV$gke-%PROC%%X%-%C%%D%%T%%G%%R%%E%.hex
echo compiled - UAVP-V3.15.$WCREV$gke-%PROC%%X%-%C%%D%%T%%G%%R%%E%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVP-V3.15.$WCREV$gke-%PROC%%X%-%C%%D%%T%%G%%R%%E%.hex
echo failed - UAVP-V3.15.$WCREV$gke-%PROC%%X%-%C%%D%%T%%G%%R%%E%.hex >> gen.lst
